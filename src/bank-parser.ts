...
export async function parseAccountFile(fileName: string): Promise<string[]> {
...
export function parseLCDLine([line1, line2, line3]: string[]) {
  ...
  for (let i = 0; i < line1.length; i += 4) {
    lcdNumbers.push(
      line1.substr(i, 3) + line2.substr(i, 3) + line3.substr(i, 3),
    );
  }
...
export function parseNumber(inputLcdNumber: string) {
  const number = lcdNumbers.findIndex(
    lcdNumber => lcdNumber === inputLcdNumber,
  );
